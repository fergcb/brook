import { ArrayExpression, AssignmentExpression, FunctionCallExpression, LiteralExpression } from "./ast.js";

export function parse(tokens) {
  tokens.reverse()
  const [program, rest] = parseProgram(tokens)
  if (program === null) throw new Error("Failed to parse program.")
...
function parseAssignment(tokens) {
  ...
  ; [expr, rest] = parseRHS(tokens)
  if (rest === null) return [null, tokens]

  if (rest[0]?.kind !== "ARROW") return [null, tokens]
  rest = rest.slice(1)

  ; [name, rest] = parseIdent(rest)
  if (name === null) return [null, tokens]

  return [new AssignmentExpression(name, expr), rest]
}

function parseExpression(tokens) {
  let [match, rest] = parseAssignment(tokens)
  if (match !== null) return [match, rest]

  ; [match, rest] = parseBinaryFunctionCall(tokens)
  if (match !== null) return [match, rest]
...
function parseProgram(tokens) {
  let match, rest = tokens, exprs = [] 
  do {
    if (rest[0]?.kind !== "SEMICOLON") return [null, tokens]
    rest = rest.slice(1)
